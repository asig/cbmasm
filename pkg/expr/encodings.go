...
var (
	...
	// PETSCII to Screen codes
	petToScreen []byte
)

func init() {
	// Based on https://sta.c64.org/cbm64pettoscr.html
	petToScreen = make([]byte, 256)
	for i := 0; i < 32; i++ {
		petToScreen[i] = byte(i + 128)
	}
	for i := 32; i < 64; i++ {
		petToScreen[i] = byte(i)
	}
	for i := 64; i < 96; i++ {
		petToScreen[i] = byte(i - 64)
	}
	for i := 96; i < 128; i++ {
		petToScreen[i] = byte(i - 32)
	}
	for i := 128; i < 160; i++ {
		petToScreen[i] = byte(i + 64)
	}
